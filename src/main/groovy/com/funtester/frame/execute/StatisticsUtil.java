...
public class StatisticsUtil extends Constant {
    ...
    public static String statistics(List<Integer> data, String title, int threadNum) {
        ...
        return draw(nums, StringUtil.center(((StringUtils.isEmpty(title)) ? DEFAULT_STRING : getTrueName(title) + TAB + (threadNum == 0 ? EMPTY : threadNum) + " thread"), BUCKET_SIZE * 3) + LINE + LINE + StringUtil.center("Response Time: x-serial num, y-median", BUCKET_SIZE * 3) + LINE + StringUtil.center("min median:" + nums.get(0) + " ms,max:" + nums.get(BUCKET_SIZE - 1) + " ms", BUCKET_SIZE * 3));
    }
